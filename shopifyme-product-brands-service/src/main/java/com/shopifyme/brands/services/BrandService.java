...
public class BrandService {
	...
	@Autowired
	private CategoriesServiceCommunicator cateogiresApiCommunicator;
		...
		if(brandDomainsList != null && brandDomainsList.size() >0) {
			...
			brandDomainsList.stream().forEach(brandDomain->{
				...
				try {
					...
					// TODO Auto-generated catch blockclou
					e.printStackTrace();
				}
	...
	public String fetchAllCategories(){
		//return cateogiresApiCommunicator.fetchAllCategories();
		Map<String,String> headerMap = new HashMap<>();
		headerMap.put(HttpHeaders.CONTENT_TYPE, MediaType.APPLICATION_JSON_VALUE);
		ResponseEntity<String> resp = categoriesFeignClient.fetchAllCategories(headerMap, Sort.DEFAULT_DIRECTION.name());
